    ...
    public static void onCommand(CommandSender sender, String[] args) {
        if (args.length == 1){
            sender.sendMessage(MessageFormat.getMessageDefault("""
                ...
                &6/cdk remove <cdk>  -->  删除一个 CDK
                &6/cdk autoremove  -->  删除已过期或用完的 CDK (此操作会重载插件)
                &6/cdk get <cdk>  -->  领取一个 CDK
                &6/cdk list [cdk]  -->  查看(指定) CDK (的信息)列表
                &6/cdk list ? page  -->  查看 CDK 列表的指定页面
                &6/cdk info  -->  查看插件信息
        ...
        switch (args[1]) {
            case "create" -> sender.sendMessage(MessageFormat.getMessageDefault("""
                    &b=======-------    &3Duomi-CDK 帮助列表    &b-------=======
                    &6create long <count> <reward> [1m/1h/1d/inf] [use] [filename]  -->  创建长 CDK
                    &6create medium <count> <reward> [1m/1h/1d/inf] [use] [filename]  -->  创建中 CDK
                    &6create short <count> <reward> [1m/1h/1d/inf] [use] [filename]  -->  创建短 CDK
                    &6create custom <cdk> <reward> [1m/1h/1d/inf] [use]  -->  创建自定义 CDK
                    &b=======-------    &3Duomi-CDK 帮助列表    &b-------=======
                    """));
            case "reward" -> sender.sendMessage(MessageFormat.getMessageDefault("""
                    ...
                    &6reward list ? page  -->  查看奖励列表的指定页面
                    &6reward clear <name>  -->  清空指定奖励内的所有 CDK
                    &b=======-------    &3Duomi-CDK 帮助列表    &b-------=======
                    """));
